...
''',
    '''  if(!report)throw new Error("Processing ended without a report.");
  // Model-backed runs keep the server's authoritative processing time. Direct
  // CSV/spreadsheet uploads use the browser wall clock only when it is absent.
...
Path("tests/test_demo_progress_ui.py").write_text(
    '''from pathlib import Path
